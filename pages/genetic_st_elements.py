...
from src.functions import func_callables, func_minimas
...
def real_inputs(func, npop):
    ...
    with col3:
        maxgen = st.number_input('Maximum number of generations', min_value = 1, max_value = 200, value = 20, step = 5)
    with col4:    
    ...
    return (maxgen, n_tourn, pc, pm, eta)
...
def binary_inputs(func, npop):
    ...
    with col4:    
        nbits = st.number_input('Number of Enconding Bits', min_value = 5, max_value = 20, value = 10, step = 1)
    col5, col6 = st.beta_columns(2)
    with col5:    
        pc = st.number_input('Crossover Probability', min_value = 0.0, max_value = 1.0, value = 0.9, step = .05)
    with col6:    
        n_tourn = st.number_input('Competitors in tournament selection', min_value = 2, max_value = 10, value = 2, step = 1)
    return (maxgen, n_tourn, pc, nbits)
...
def compare_results(selec_func, df):
    ...
    try:
        ...
        abs_diff = '-'
    col_teor, col_result, col_error = st.beta_columns(3)
    with col_teor:
